...
**Mục tiêu portfolio:** có một demo chạy được, kết quả tái lập được và phần trình bày nêu
đúng metric, điều kiện thí nghiệm và giới hạn cho vị trí Data Analyst (DA), Data Scientist
(DS) và AI Engineer (AIE).
...
| `report/week-01/` | EDA, daily log, baseline results | Bằng chứng tiến độ tuần 1 | **Có.** Báo cáo này nối tiếp và không ghi lại lịch sử sai khác. |
...
| API, Docker, CI | Chưa tồn tại | Là scope AIE tối thiểu ở Sprint 3; dashboard tĩnh vẫn là fallback demo hợp lệ. |
...
| Causal Forest main tối đa 50% | Kaggle Free **chỉ nếu** cả hai preflight qua | Resource-gated profile: 200 cây, CV=2, inference=False | Nếu 30% không qua gate, giữ release 5-model; không nâng hạ tầng chỉ để đủ model thứ sáu. |
...
| 3. Causal Forest decision gate | Chạy 20% rồi 30% trên Colab theo `docs/COLAB_CAUSAL_FOREST.md`; log wall time, peak RAM, errors, alignment với holdout | GPU availability khác system-RAM; benchmark/feasibility là experiment | `benchmarks/logs` + result một trong hai: (a) valid CATE 50%, hoặc (b) documented decision release 5-model. |
...
| 3. Dashboard | Dùng artifact precomputed để dashboard load nhanh: model comparison, Qini, decile table, target slider, scenario inputs, limitations/data provenance panel | DA: KPI/table/story; DS: uncertainty/causal caveat; AIE: artifact boundary và reliable UI | Dashboard chạy local; screenshot + GIF/video 60–90 giây; không hard-code result không trace được. |
...
| 5. Mini acceptance test | Test dashboard artifact schema and manually replay 3 scenario: low cost, high cost, no-target/random | Kết quả phải nhất quán với công thức ở từng scenario | `report/week-03-04-demo-checklist.md` với screenshot/expected result. |
...
### Sprint 3 — Đóng gói portfolio, kiểm chứng release và mở lối probabilistic CLV

**Thời gian:** Tuần 5–6
**Mục tiêu sprint:** tạo package show được cho DA/DS/AIE; chốt plan có căn cứ cho phase probabilistic mà không nhồi một dự án thứ hai vào deadline.
...
| 2. Final report | Viết theo Research Question → Data/identification → Methods → Results → Decision simulation → Limitations → Reproduction | DS interview đánh giá judgment, không chỉ model count | `report/final-report.md`, tất cả number link về output/run ID; source table từ mục 4. |
| 3. Portfolio narrative | README 5-minute path, architecture diagram nhỏ, 60–90s demo video, 6–8 slides; 3 CV bullets có định lượng nhưng có context | DA/DS/AIE cần evidence khác nhau nhưng cùng project | Public-ready README, video link/path, slide deck, CV bullets. |
...
- CV có thể nêu: quy mô dữ liệu, số baseline, evaluation validation, quyết định/sản phẩm, với giới hạn được minh bạch.
...
|---|---|---|---|
| 1 — đã qua | EDA, randomization diagnostic, 5 baseline, metric tests | `report/week-01/`, benchmark, result table | Response có Qini cao nhất; policy metric có phù hợp decision objective không? |
...
| 4 | Dashboard acceptance, sensitivity, user-facing explanation | Demo checklist/video draft | Mức technical/product detail cho audience tuyển dụng đã cân bằng chưa? |
...
| 6 | Release QA, demo recording, CV, phase-2 handoff | Final portfolio package | Quyết định next project: CLV baseline trước hay Hillstrom monetary causal policy trước? |

---

## 7. Tiêu chuẩn sản phẩm cho ba nhóm vị trí

| Vị trí | Người tuyển dụng cần nhìn thấy | Bằng chứng trong release này |
|---|---|---|
| Data Analyst | Metric definition, data quality, segmentation/decile insight, dashboard và stakeholder narrative | Data contract, decile table, scenario simulator, report/slide rõ ràng. |
| Data Scientist | Estimand, RCT/identification caveat, baseline comparison, uncertainty, invalid-result handling | Nghiên cứu 5/6 model, đối chiếu metric, bootstrap CI, negative control và báo cáo kết quả không cải thiện kèm artifact/CI. |
| AI Engineer | Reproducible pipeline, artifacts, test, packaging, demo reliability | Scripts, test suite, manifest, dashboard entrypoint, optional Docker/CI. |

**CV bullet mẫu (chỉ dùng sau final verification):**

- Xây pipeline uplift modeling tái lập trên Criteo RCT 14,0M records, benchmark 5–6 response/CATE learners và kiểm chứng Qini/AUUC với `scikit-uplift`, kèm 500× percentile-bootstrap uncertainty.
- Chuyển CATE ranking thành Uplift Targeting Console: decile policy, sensitivity theo contact cost/contribution-value assumption, data provenance và export artifact có run ID.
- Đóng gói mô hình thành demo có tests, run manifest và runbook; báo cáo cả trường hợp response baseline vượt causal meta-learner trên Qini thay vì cherry-pick kết quả.

Không ghi “tăng doanh thu X%”, “production-ready” hoặc “CLV optimization” cho release causal nếu không có online experiment/revenue/production evidence.
...
- [ ] CV bullets chỉ dùng số có manifest/artifact hỗ trợ.
...
3. Chấp nhận roadmap tách pha: causal portfolio release trước; CLV/iCLV sau release, dùng Online Retail II + Hillstrom đúng vai trò dữ liệu.
