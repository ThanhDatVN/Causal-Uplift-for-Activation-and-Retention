...
def early_stop_reason(score: np.ndarray, protocol: dict) -> str | None:
    ...
    with ResourceMonitor(
        ...
        for spec in specs:
            ...
            base = base_registry_fields(
                run_id=f"{protocol['protocol_id']}-{args.stage}-{spec.name}",
                status=args.stage,
            ...
            score = result["score"]
            stop_reason = early_stop_reason(score, protocol)
            base.update(
                {
